...
export const getOwnerEarningsOverview = asyncHandler(async (req, res) => {
    ...
    const propertyIds = myProperties.map(p => p._id);
    const currentDate = new Date(); // Current system timestamp

    // 2. Query all historical bookings that generated or held value for this owner:
    const allBookings = await Booking.find({
        property: { $in: propertyIds },
        $or: [
            { status: "confirmed" },
            { status: "cancelled", ownerEarnings: { $gt: 0 } }
        ]
    }).select("property totalPrice ownerEarnings status startDate endDate");

    // 3. Helper: Strict Escrow & Checkout-Completion Rule
    const getUnlockedRevenueValue = (booking) => {
        // Rule A: If cancelled mid-stay or late, it uses the processed/adjusted owner earnings record
        if (booking.status === "cancelled") {
            return booking.ownerEarnings !== undefined ? booking.ownerEarnings : 0;
        }

        // Rule B: If confirmed, check if checkout date has actually completed
        if (booking.status === "confirmed") {
            const checkOutDate = new Date(booking.endDate || booking.checkOut);
            
            // If the checkout date has passed, the money unlocks from escrow!
            if (currentDate >= checkOutDate) {
                return booking.totalPrice || 0;
            }
            
            // Otherwise, stay is upcoming/ongoing -> Keep locked in escrow ($0 counted yet)
            return 0;
        }
    ...
    // Filter to only count bookings that have successfully unlocked their revenue
    const unlockedBookings = allBookings.filter(b => getUnlockedRevenueValue(b) > 0);

    // 4. LAYER A: Compute Grand Total Revenue from unlocked/completed stays only
    const grandTotalEarnings = allBookings.reduce(
        (sum, booking) => sum + getUnlockedRevenueValue(booking), 0
    );

    // 5. LAYER B: Compile itemized Property-by-Property Breakdown array
    const propertyPerformanceBreakdown = myProperties.map(property => {
        const structuralBookings = allBookings.filter(
            b => b.property.toString() === property._id.toString()
        );

        const individualPropertyEarnings = structuralBookings.reduce(
            (sum, b) => sum + getUnlockedRevenueValue(b), 0
        );

        const unlockedCountForProperty = structuralBookings.filter(
            b => getUnlockedRevenueValue(b) > 0
        ).length;
        ...
        return {
            ...
            totalBookingsCount: unlockedCountForProperty,
    ...
    return res.status(200).json({
        success: true,
        message: "Owner escrow checkout-completion earnings matrix compiled successfully.",
        analytics: {
            grandTotalEarnings, // Only reflects completed stays or adjusted cancellations
            totalConfirmedBookingsCount: unlockedBookings.length,
            propertyPerformanceBreakdown 
        }
    });
});
